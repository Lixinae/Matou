...
public class Client {
	...
	private BlockingQueue<String> queueConnect = new ArrayBlockingQueue<String>(
			1);
	...
	private BlockingQueue<String> queueClient = new ArrayBlockingQueue<String>(
			1);
	private BlockingQueue<ByteBuffer> queueServer = new ArrayBlockingQueue<ByteBuffer>(
			1);
	...
	private void sendInfoServer() {
		...
		try {
			...
			System.err
					.println("Erreur lors de l'écriture d'un paquet du serveur , Serveur deconnecter");
			exitClient();
			end = true;
			return;
		}
	...
	private boolean sendPseudo() {
		...
		if (scan.hasNextLine()) {
			...
			bNickNameToServer.put(E_PSEUDO).putInt(tmp.length()).put(bNickName);
	...
	private void sendMessageAll(String messageAll) {
		...
		buffSendAll.put(M_ALL).putInt(nickname.length()).put(buffNickName)
				.putInt(messageAll.length()).put(buffMessage);
		buffSendAll.flip();
		...
		try {
			...
			System.err
					.println("Erreur lors de l'envoie du message, serveur deconnecter");
			exitClient();
		}
	}
	...
	private void sendPseudoAck(String pseudoACK) {
		...
		buffSendACK.put(ACK_CO_CLIENT).putInt(nickname.length())
	...
	private void sendPseudoConnect(String pseudoConnect) {
		...
		buffConnect.put(CO_CLIENT_TO_CLIENT).putInt(pseudoConnect.length())
		...
		try {
			...
			System.err
					.println("Erreur lors de l'envoie de la demande de connection, serveur deconnecter");
			exitClient();
		}
	}
	...
	private void sendMessageToClient(String messageToClient, String dest) {
		...
		buffSend.put(M_CLIENT_TO_CLIENT).putInt(messageToClient.length())
		...
		friends.forEach((name, socketFriend) -> {
			if (name.equals(destCpy)) {
				try {
					...
					System.err
							.println("Erreur lors de l'envoie du message au client "
									+ destCpy);
				}
			}
		});
	...
	private void demandeList() {
		...
		try {
			...
			System.err
					.println("Erreur lors de l'envoie de la demande liste, serveur deconnecter");
		}
	}
